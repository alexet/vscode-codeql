...
import { decodeSourceArchiveUri, sourceArchiveScheme } from "./archive-filesystem-provider";
...
async function resolveQueries(keyType: KeyType): Promise<string[]> {
  switch (keyType) {
    case KeyType.DefinitionQuery: return ["/home/alex/code/ql/cpp/ql/src/localDefinitions.ql"]
    case KeyType.ReferenceQuery: return ["/home/alex/code/ql/cpp/ql/src/localReferences.ql"]
  }
}
...
export function createDefinitionsHandler(cli: CodeQLCliServer, qs: QueryServerClient, dbm: DatabaseManager): vscode.DefinitionProvider {
  let fileCache = new CachedOperation<vscode.LocationLink[]>(async (uriString: string) => {
    ...
    const sourceArchiveUri = vscode.Uri.file(uri.sourceArchiveRootPath).with({ scheme: sourceArchiveScheme });
